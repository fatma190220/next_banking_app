...
const CategoryBadge = ({ category }: CategoryBadgeProps) => {
  ...
  return (
    <div className={cn("flex items-center justify-center  truncate w-fit gap-1 rounded-2xl border-[1.5px] py-[2px] pl-1.5 pr-[2px]",  borderColor, chipBackgroundColor)}>
        ...
        {(transactions ?? []).map((t: Transaction) => {
          ...
          return (
            <TableRow key={t.id} className={`${isDebit || amount[0] === '-' ? 'bg-[#FFFBFA]' : 'bg-[#F6FEF9]'} !over:bg-none !border-b-DEFAULT`}>
              ...
              <TableCell className="pl-3 pr-10">
